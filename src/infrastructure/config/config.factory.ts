...
export function createServerConfig(): ServerConfig {
    ...
    debugConfig('Server configuration parsed successfully: port=%d, redisUrl=%s', parsedConfig.port, parsedConfig.redisUrl);
...
export function createClientConfig(): ClientConfig {
    ...
    const config = {
        url: process.env['URL'],
        reconnectionDelay: process.env['RECONNECTION_DELAY'] !== undefined ? Number(process.env['RECONNECTION_DELAY']) : undefined,
        connectionTimeout: process.env['CONNECTION_TIMEOUT'] !== undefined ? Number(process.env['CONNECTION_TIMEOUT']) : undefined,
